...
class PokerActorCritic(nn.Module):
    ...
    def __init__(
        ...
        num_heads: int = 8,
        num_layers: int = 4,
        num_actions: int = NUM_ACTIONS,  # fold, check, call, 9 raise sizes, all-in (unified action space)
        dropout: float = 0.1,
        ...
        # Action space prior - DISABLED (v3)
        # 
        # Previous versions used logit biases to "balance" probability mass:
        # - fold/check/call got positive biases (+0.66 to +0.88)
        # - all_in got negative bias (-0.5)
        # 
        # PROBLEM: These hardcoded biases persisted after training, making the model
        # overly passive. Even after thousands of training iterations, the model
        # couldn't overcome architectural biases toward check/call.
        #
        # FIX: Set all priors to 0. Let the model learn entirely from rewards.
        # The policy head will learn appropriate action preferences through training.
        action_prior = torch.zeros(num_actions)
        
        self.register_buffer('action_prior', action_prior)
        
        # Hand-strength-aware action gating (LEARNED, not hardcoded)
        # This network learns contextual adjustments based on hand strength
        # CRITICAL: It should LEARN when aggression is good/bad, not be hardcoded
        ...
        # DISABLED: Hardcoded hand strength threshold penalties
        # Previous bug: These hardcoded penalties conflicted with learning from rewards.
        # The model was penalized at the ARCHITECTURE level for all-ins with weak hands,
        # even when such all-ins were profitable (e.g., successful bluffs).
        # Now the model learns entirely from chip outcomes when to be aggressive.
        self.hand_strength_threshold = None  # DISABLED - let model learn from results
    ...
    def forward(
        ...
        # Apply hand-strength-aware action gating (LEARNED only, no hardcoded penalties)
        # The gate network learns contextual adjustments based on hand strength and state
        gate_input = torch.cat([pooled, hand_strength], dim=-1)
        action_gate = self.hand_strength_gate(gate_input)  # (batch_size, num_actions)
        
        # DISABLED: Hardcoded all-in penalties based on hand strength
        # Previous bug: These penalties conflicted with reward-based learning.
        # The model now learns entirely from outcomes (chip EV) when aggression is correct.
        # 
        # OLD CODE (REMOVED):
        # weak_hand_mask = (hand_strength < self.hand_strength_threshold).float()
        # all_in_penalty[:, 12] = -1.0 * weak_hand_mask.squeeze()
        # for raise_idx in [8, 9, 10, 11]:
        #     all_in_penalty[:, raise_idx] = -0.3 * weak_hand_mask.squeeze()
        
        # Apply only learned gate (no hardcoded penalties)
    ...
    def forward_with_hand_strength(
        ...
        # DISABLED: Hardcoded penalties (see forward() for explanation)
        # Let the model learn from chip outcomes when aggression is appropriate
        action_logits = action_logits + action_gate
