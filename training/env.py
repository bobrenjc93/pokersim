...
# Import from common package for shared simulation logic
from common import (
...
class VecPokerEnv:
    ...
    This environment uses the C++ poker_api_binding directly for maximum
    performance in vectorized training. For simpler use cases, see
    common.simulation.DirectGameSimulator.
    
    Uses common.GameConfig and common.create_binding_config for consistent
    configuration handling across the codebase.
    """
    ...
    def __init__(
        ...
        """
        Initialize vectorized poker environment.
        
        Args:
            num_envs: Number of parallel environments
            config: Game configuration - either GameConfig instance or dict
            opponent_pool: List of opponent agents to sample from
        ...
        # Convert to GameConfig if dict provided (uses common consolidation)
        if isinstance(config, dict):
            self.game_config_obj = GameConfig.from_dict(config)
        else:
            self.game_config_obj = config
        
        # Store original config dict for backward compatibility
        self.config = self.game_config_obj.to_dict()
        
        # Create binding config template (seed will be set per-game in _reset_env)
        # We'll create new binding configs per game to have different seeds
        self._binding_config_template = self.game_config_obj
    ...
    def _reset_env(self, env_idx: int):
        ...
        # Create binding config with new random seed using common helper
        binding_config = create_binding_config(
            self._binding_config_template, 
        ...
        # Setup players
        # Heads-up: p0 and p1
        game.add_player("p0", "Player 0", self.game_config_obj.starting_chips)
        game.add_player("p1", "Player 1", self.game_config_obj.starting_chips)
    ...
    def step(self, actions: List[str]) -> Tuple[List[Dict], List[float], List[bool], List[Dict]]:
        ...
        for i in range(self.num_envs):
            ...
            if game.get_stage_name() == "Complete":
                ...
                profit = hero_chips - self.game_config_obj.starting_chips
                rewards[i] = profit / self.game_config_obj.starting_chips
