...
from common import create_actor_critic, RLStateEncoder, GameConfig, create_binding_config
...
def benchmark_direct_binding(num_episodes: int = 100):
    ...
    # Convert to common GameConfig for consistent handling
    game_cfg = GameConfig.from_dict(game_config)
    
    for _ in range(num_episodes):
        # Create game using common simulation utilities
        binding_config = create_binding_config(game_cfg, seed=random.randint(0, 1000000))
        
        game = poker_api_binding.Game(binding_config)
        game.add_player('p0', 'Player_p0', game_cfg.starting_chips)
        game.add_player('p1', 'Player_p1', game_cfg.starting_chips)
        game.start_hand()
