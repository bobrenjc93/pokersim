...
# Import RL components from common package
from common import (
    # State encoding
    RLStateEncoder,
    ...
    LOG_LEVEL,
    DEFAULT_MODELS_DIR,
    # Simulation utilities (shared with ELO)
    GameConfig,
...
class RLTrainingSession:
    ...
    def collect_episode_fast(
        ...
        # Create game using common simulation utilities (shared with ELO)
        game_cfg = GameConfig.from_dict(self.game_config)
        binding_config = create_binding_config(game_cfg, seed=random.randint(0, 1000000))
        
        game = poker_api_binding.Game(binding_config)
        
        # Add players
        for agent in agents:
            game.add_player(agent['id'], f"Player_{agent['id']}", game_cfg.starting_chips)
    ...
    def play_hand_for_freezeout(
        ...
        # Add players
        game.add_player('p0', 'Agent', game_cfg.starting_chips)
        game.add_player('p1', 'Opponent', game_cfg.starting_chips)
        game.start_hand()
    ...
    def _call_api(self, history: List[Dict]) -> Dict:
        """Call poker API using common simulation module."""
        return call_poker_api(self.game_config, history)
