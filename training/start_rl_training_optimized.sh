...
if [ -z "${MODEL_VERSION:-}" ]; then
    # Extract from common/config.py (the source of truth)
    COMMON_CONFIG="$(dirname "${SCRIPT_DIR}")/common/config.py"
    if [ -f "$COMMON_CONFIG" ]; then
        MODEL_VERSION=$(grep "^MODEL_VERSION =" "$COMMON_CONFIG" | awk '{print $3}' | tr -d ' ')
    fi
fi
...
echo -e "${BLUE}📦 Installing common package...${NC}"
